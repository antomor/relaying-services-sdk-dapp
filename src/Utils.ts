...
class Utils {
  static async ritTokenDecimals() {
    const rifTokenContract = new web3.eth.Contract(TestToken.abi, process.env.REACT_APP_CONTRACTS_RIF_TOKEN);
...
async function estimateDestinationContractCallGas(transactionDetails: EnvelopingTransactionDetails, addCushion = true): Promise<string|number> {
  ...
  let internalCallCost = estimated > INTERNAL_TRANSACTION_ESTIMATE_CORRECTION
...
export async function estimateMaxPossibleRelayGas(relayClient: RelayClient, trxDetails: EnvelopingTransactionDetails) {
  ...
  const maxPossibleGasValue = await relayClient.estimateMaxPossibleRelayGas(txDetailsClone, process.env.REACT_APP_CONTRACTS_RELAY_WORKER!);
